...
const char* mqtt_server = "172.20.49.75";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (message == "ON" && !motorActivated) {
    ...
    rotateMotorAsync(totalSteps, false);  // 시계 방향 회전 (direction = true)
  ...
  // OFF 메시지를 받으면 모터 전류를 끊고 잠시 후에 반시계 방향으로 회전
    ...
    rotateMotorAsync(totalSteps, true);  // 반시계 방향 회전 (direction = false)
